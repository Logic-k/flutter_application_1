...
List<String> _currentOptions() {
  return find
      .descendant(of: find.byType(OutlinedButton), matching: find.byType(Text))
      .evaluate()
...
void main() {
  ...
  late _MockTrainingProgressProvider mockProgress;
  ...
  setUp(() {
    ...
    mockProgress = _MockTrainingProgressProvider();
    // 레벨 1 → easy 문제풀, 3지선다
    when(() => mockDifficulty.getLevel(any())).thenReturn(1);
    when(() => mockDifficulty.getTargetTime(any())).thenReturn(5.0);
    when(
      () => mockDifficulty.updatePerformance(
        any(),
        any(),
        responseTime: any(named: 'responseTime'),
      ),
    ).thenAnswer((_) async {});
    when(() => mockProgress.userId).thenReturn(1);
  });

  Widget subject() => MultiProvider(
    providers: [
      ChangeNotifierProvider<DifficultyProvider>.value(value: mockDifficulty),
  ...
  testWidgets('CategorizationGame: 틀린 답을 탭하면 오답 SnackBar를 표시한다', (
    ...
    final word = _currentWord();
    final wrong = _currentOptions().firstWhere((o) => o != _categoryOf[word]);
  ...
  testWidgets('CategorizationGame: 저장 실패 시 같은 시도를 재시도한다', (tester) async {
    when(
      () => mockProgress.complete(any()),
    ...
    final firstInput =
        verify(() => mockProgress.complete(captureAny())).captured.single
            as TrainingAttemptInput;
    for (
      var i = 0;
      i < 10 && find.text('기록을 저장하지 못했습니다. 다시 시도해 주세요.').evaluate().isEmpty;
      i++
    ) {
      await tester.pump(const Duration(milliseconds: 100));
    }
    expect(find.text('기록을 저장하지 못했습니다. 다시 시도해 주세요.'), findsOneWidget);
    expect(firstInput.activityId, 'categorization');
    expect(firstInput.score, 100.0);
    expect(firstInput.correctAnswers, 10);
    expect(firstInput.totalQuestions, 10);
    ...
    expect(retriedInput.score, firstInput.score);
  });
