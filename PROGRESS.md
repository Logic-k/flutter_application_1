...
> **2026-07-23 인지훈련 게임화**:
> - 8개 활동 과정 지도, XP(100 XP당 레벨), 숙련도 1~3, 하루 3개 서로 다른
>   활동 목표, 서울 날짜 기준 연속 학습, 선행 활동 잠금 해제를 구현했다.
> - 완료는 UUID 시도 ID 기반 단일 트랜잭션으로 저장하며 중복 제출은 XP와
>   임상 점수를 다시 기록하지 않는다.
> - **SQLite v9** 4개 진행 테이블을 추가했다. v8 기존 사용자는 8개 활동을
>   유지하고, 신규/초기화 사용자는 시작 활동 5개를 받는다.
> - 임상 `training_scores`와 격려용 XP/숙련도/연속 학습은 분리한다.
> - 게임화 API 연동은 이번 범위에서 보류한다. 로컬 저장소에 HTTP 또는
>   Firestore 동기화 구현을 추가하지 않았다.
> - CI는 Flutter 3.41.5, Android 통합 기준은 API 33, 로컬 릴리스 점검은
>   `QA_Device` API 36이다.
>
